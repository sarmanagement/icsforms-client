...
/**
 * Modal dialog that lists locally available incident workspace files for the
 * operator to choose from.
 *
 * <p>
 * The dialog scans a known local directory (typically {@code ~/.icsforms}) for
 * JSON workspace files, shows a summary list sorted by last-modified time, and
 * lets the operator either pick an entry from the list or browse the filesystem
 * for a file that is not in the list. This design is intended to generalise in
 * future to remote incident repositories.
 * </p>
 *
 * <p>
 * Usage:
...
public class IncidentPickerDialog extends JDialog {
	 ...
	 * @param localDir
	 *            directory to scan for saved incidents (typically
	 *            {@code ~/.icsforms}).
	 */
	public IncidentPickerDialog(Window owner, Path localDir) {
		...
		String[] labels = summaries.stream().map(s -> buildLabel(s)).toArray(String[]::new);
	...
	/**
	 * Returns the path chosen by the operator, or {@code null} when the dialog was
	 * cancelled.
	 *
