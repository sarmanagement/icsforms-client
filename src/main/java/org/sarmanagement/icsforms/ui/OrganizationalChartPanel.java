 ...
 *
 * <p>
 * Each staff position shows a Name field and — visible only when the name is
 * non-blank — separate Radio and Phone fields. Positions are grouped into:
 * Incident Command, Command Staff, General Staff, Planning Section Positions,
 * and Logistics Section Positions.
 * </p>
 */
public class OrganizationalChartPanel extends JPanel {
	...
	private final DefaultTableModel customPositionsModel = new DefaultTableModel(CUSTOM_COLS, 0) {
		@Override
	...
	public OrganizationalChartPanel(AppController controller) {
		...
		JPanel generalSection = buildSection("General Staff",
				new String[]{"Operations Section Chief", "Planning Section Chief", "Logistics Section Chief",
		...
		JPanel planningSection = buildSection("Planning Section Positions",
				new String[]{"Documentation Unit Leader", "Resources Unit Leader", "Situation Unit Leader",
		...
		JPanel opsSection = buildSection("Operations Section Positions", new String[]{"Staging Area Manager"},
				new JTextField[]{stagingAreaManagerNameField}, new JTextField[]{stagingAreaManagerRadioField},
				new JTextField[]{stagingAreaManagerPhoneField});

		JPanel logisticsSection = buildSection("Logistics Section Positions",
				new String[]{"Communications Unit Leader", "Communications Technician", "Supply Unit Leader",
						"Facilities Unit Leader", "Ground Support Unit Leader", "Food Unit Leader"},
				new JTextField[]{commUnitLeaderNameField, commTechNameField, supplyUnitLeaderNameField,
						facilitiesUnitLeaderNameField, groundSupportUnitLeaderNameField, foodUnitLeaderNameField},
				new JTextField[]{commUnitLeaderRadioField, commTechRadioField, supplyUnitLeaderRadioField,
						facilitiesUnitLeaderRadioField, groundSupportUnitLeaderRadioField, foodUnitLeaderRadioField},
				new JTextField[]{commUnitLeaderPhoneField, commTechPhoneField, supplyUnitLeaderPhoneField,
						facilitiesUnitLeaderPhoneField, groundSupportUnitLeaderPhoneField, foodUnitLeaderPhoneField});

		JPanel financeSection = buildSection("Finance / Admin Section Positions",
				new String[]{"Time Unit Leader", "Procurement Unit Leader", "Comp / Claims Unit Leader",
						"Cost Unit Leader"},
				new JTextField[]{timeUnitLeaderNameField, procurementUnitLeaderNameField, compClaimsUnitLeaderNameField,
						costUnitLeaderNameField},
		...
		installPersonAutocomplete(operationsSectionChiefField, operationsSectionChiefRadioField,
				operationsSectionChiefPhoneField);
		installPersonAutocomplete(planningSectionChiefNameField, planningSectionChiefRadioField,
				planningSectionChiefPhoneField);
		installPersonAutocomplete(logisticsSectionChiefNameField, logisticsSectionChiefRadioField,
				logisticsSectionChiefPhoneField);
		installPersonAutocomplete(financeAdminSectionChiefNameField, financeAdminSectionChiefRadioField,
				financeAdminSectionChiefPhoneField);
		installPersonAutocomplete(documentationUnitLeaderNameField, documentationUnitLeaderRadioField,
				documentationUnitLeaderPhoneField);
		installPersonAutocomplete(resourcesUnitLeaderNameField, resourcesUnitLeaderRadioField,
				resourcesUnitLeaderPhoneField);
		installPersonAutocomplete(situationUnitLeaderNameField, situationUnitLeaderRadioField,
				situationUnitLeaderPhoneField);
		installPersonAutocomplete(demobUnitLeaderNameField, demobUnitLeaderRadioField, demobUnitLeaderPhoneField);
		installPersonAutocomplete(stagingAreaManagerNameField, stagingAreaManagerRadioField,
				stagingAreaManagerPhoneField);
		...
		installPersonAutocomplete(facilitiesUnitLeaderNameField, facilitiesUnitLeaderRadioField,
				facilitiesUnitLeaderPhoneField);
		installPersonAutocomplete(groundSupportUnitLeaderNameField, groundSupportUnitLeaderRadioField,
				groundSupportUnitLeaderPhoneField);
		...
		installPersonAutocomplete(procurementUnitLeaderNameField, procurementUnitLeaderRadioField,
				procurementUnitLeaderPhoneField);
		installPersonAutocomplete(compClaimsUnitLeaderNameField, compClaimsUnitLeaderRadioField,
				compClaimsUnitLeaderPhoneField);
	...
	/**
	 * Builds the custom positions section with an editable table and Add/Remove
	 * buttons.
	 */
	private JPanel buildCustomPositionsPanel() {
		...
		JComboBox<String> sectionCombo = new JComboBox<>(
				new String[]{"Command Staff", "Operations", "Planning", "Logistics", "Finance/Admin"});
	...
	private void addCustomPosition() {
		...
		pickButton.addActionListener(
				e -> pickPersonIntoFields("Select Position Resource", nameField, radioField, phoneField));
		...
		int result = JOptionPane.showConfirmDialog(this, form, "Add Custom Position", JOptionPane.OK_CANCEL_OPTION,
				JOptionPane.PLAIN_MESSAGE);
		if (result == JOptionPane.OK_OPTION) {
			customPositionsModel.addRow(new Object[]{sectionBox.getSelectedItem(), titleField.getText().trim(),
					nameField.getText().trim(), radioField.getText().trim(), phoneField.getText().trim()});
		}
	}

	/**
	 * Installs autocomplete on {@code nameField} and hides the radio/phone fields
	 * when the name is blank. When a known T-card name is selected the radio field
	 * is auto-filled (preferred) or the phone field (fallback) if both are
	 * currently blank.
	 */
	private void installPersonAutocomplete(JTextField nameField, JTextField radioField, JTextField phoneField) {
		...
		UiSupport.installNameAutocomplete(nameField, () -> controller.getPersonnelNames(), selectedName -> {
	...
	public void refreshFromModel() {
		...
		if (chart.getIncidentCommanderEntries() != null && !chart.getIncidentCommanderEntries().isEmpty()) {
			for (OrgChartEntry entry : chart.getIncidentCommanderEntries()) {
				addIncidentCommanderRow(safe(entry.getName()), safe(entry.getRadio()), safe(entry.getPhone()),
			...
			for (int i = 0; i < names.size(); i++) {
				addIncidentCommanderRow(safe(names.get(i)), i == 0 ? safe(chart.getIncidentCommanderRadio()) : "",
		...
		while (customPositionsModel.getRowCount() > 0)
			customPositionsModel.removeRow(0);
		for (OrgChartEntry entry : chart.getAdditionalPositions()) {
			customPositionsModel.addRow(new Object[]{sectionLabel(entry.getSection()), safe(entry.getTitle()),
					safe(entry.getName()), safe(entry.getRadio()), safe(entry.getPhone())});
		}
	...
	public void pushToModel() {
		...
		if (customPositionsTable.isEditing())
			customPositionsTable.getCellEditor().stopCellEditing();
		...
		form207.setPreparedDateTime(
				AppController.toLocalDateTime((java.util.Date) ics207PreparedDateTimeSpinner.getValue()));
	}
	...
	private void pickPersonIntoFields(String dialogTitle, JTextField nameField, JTextField radioField,
		...
		int choice = JOptionPane.showConfirmDialog(this, combo, dialogTitle, JOptionPane.OK_CANCEL_OPTION);
		...
		if ("<Add new resource…>".equals(selected)) {
			...
			if (!UiSupport.showResizableConfirmDialog(this, "Add Resource", pane, new Dimension(540, 240))) {
				return;
			}
	 ...
	 * Radio and Phone cells are initially hidden when the corresponding Name is
	 * blank.
	 */
	private JPanel buildSection(String title, String[] labels, JTextField[] nameFields, JTextField[] radioFields,
		...
		for (int i = 0; i < labels.length; i++) {
			...
			c.gridx = 2;
			panel.add(radioFields[i], c);
			c.gridx = 3;
			panel.add(phoneFields[i], c);
		}
		return panel;
	}
	...
	private static OrgChartEntry.Section sectionFromLabel(String label) {
		if (label == null)
			return OrgChartEntry.Section.OPERATIONS;
		return switch (label) {
	...
	private record IncidentCommanderRow(JTextField nameField, JTextField radioField, JTextField phoneField,
			JCheckBox propagateCheckbox) {
	}
}
