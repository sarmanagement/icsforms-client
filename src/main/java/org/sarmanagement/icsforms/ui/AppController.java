...
/**
 * Coordinates UI edits, autosave, validation, and linked SAR task
 * synchronization.
 */
public class AppController {
	...
	public enum LinkSource {
		NONE, SHARED, ORG_CHART, ICS202, ICS204, ICS214
	}
	...
	/**
	 * Returns a sorted list of distinct personnel names from all T-cards with a
	 * non-blank name.
	 *
	 * @return sorted list of personnel names.
	 */
	public List<String> getPersonnelNames() {
		return data.getTCards().stream().filter(c -> !c.getPersonName().isBlank()).map(TCard::getPersonName).distinct()
				.sorted().toList();
	}

	/**
	 * Finds the first T-card whose {@code personName} matches {@code name}
	 * (case-insensitive).
	 *
	 * @param name
	 *            person name to look up; {@code null} or blank returns
	 *            {@code null}.
	...
	public TCard findPersonCard(String name) {
		...
		return data.getTCards().stream().filter(c -> c.getCardType() == null || c.getCardType() == TCardType.PERSONNEL)
				.filter(c -> key.equals(c.getPersonName().trim().toLowerCase())).findFirst().orElse(null);
	}

	/**
	 * Finds or creates a PERSONNEL T-card and applies provided contact details when
	 * present.
	 *
	 * @param personName
	 ...
	 * @return existing or created PERSONNEL card, or {@code null} when name is
	 *         blank.
	 */
	...
	/**
	 * Saves the active document immediately using the current authoritative
	 * linked-field source.
	 *
	 * @param source
	 *            source tab for linked role values.
	...
	/**
	 * Advances the current incident workspace to a new subsequent operational
	 * period.
	 *
	 * <p>
	 * The organizational chart and ICS 201 initial-response record are preserved as
	 * historical context. Operational-period forms are cleared so the new period
	 * starts fresh. See
	 * {@link org.sarmanagement.icsforms.model.AppData#advanceToNewOperationalPeriod()}
	 * for the full list of fields that are reset.
	 * </p>
	 */
	...
	/**
	 * Validates the active document using the current authoritative linked-field
	 * source.
	 *
	 * @param source
	 *            source tab for linked role values.
	...
	public java.util.Map<String, Path> exportSelectedForms(List<String> formKeys, Path outputDirectory,
			LinkSource source) throws IOException {
		synchronizeLinkedFields(source);
		syncSarTasks();
		syncTCards();
	...
	public Path exportSelectedIapBundle(List<String> formKeys, Path outputDirectory, LinkSource source)
			throws IOException {
		synchronizeLinkedFields(source);
		syncSarTasks();
		syncTCards();
	...
	/**
	 * Synchronizes shared org-chart-linked fields across the shared tab, org chart,
	 * ICS 202, and ICS 204.
	 *
	 * @param source
	 *            source tab for linked role values.
	...
	public void synchronizeLinkedFields(LinkSource source) {
		...
		if (source == LinkSource.SHARED && matchesOperationsSectionChiefRole(preparerTitle)
				&& !preparerName.isBlank()) {
		...
		// Sync ops section chief contact from org chart → form 204 when org chart is
		// authoritative.
		...
		if (!opsContact.isBlank()
				&& (safe(form204.getOperationsSectionChiefContact()).isBlank() || source == LinkSource.ORG_CHART)) {
		...
		if (source == LinkSource.SHARED && matchesFinanceAdminSectionChiefRole(preparerTitle)
				&& !preparerName.isBlank()) {
			organizationalChart.setFinanceAdminSectionChiefName(preparerName);
		}
		if (source == LinkSource.SHARED && matchesDocumentationUnitLeaderRole(preparerTitle)
				&& !preparerName.isBlank()) {
	...
	/**
	 * Synchronizes SAR task scaffold records from ICS 204 resources using stable
	 * assignment IDs.
	 */
	...
	/**
	 * Synchronizes editable SAR task linkage fields back into the matching ICS 204
	 * resource rows.
	 */
	public void syncIcs204ResourcesFromSarTasks() {
		...
		for (ResourceAssignment resource : data.getForm204().getResourcesAssigned()) {
			...
			// Auto-fill contact from the leader's T-card radio/phone when the field is
			// blank.
	...
	/**
	 * Synchronizes T-card records from org-chart staff positions and SAR task
	 * resources.
	 *
	 * <p>
	 * Each named staff position on the org chart and each named resource on a SAR
	 * task automatically maintains a linked T-card. A card's {@code sourceRef}
	 * field is the stable key; only the name and contact fields are updated here so
	 * that operators may freely edit status, location, and notes without losing
	 * their changes.
	 * </p>
	 *
	 * <p>
	 * Cards whose source position is later cleared (name set to blank) are removed
	 * from the list. Manually created cards (blank sourceRef) are never touched.
	 * </p>
	 */
	public void syncTCards() {
		...
		// Legacy data: old CSV imports stored the canine name in personName with a
		// blank
		// resourceIdentifier. Migrate those records now so all downstream logic can
		// rely
		// on resourceIdentifier as the authoritative display name for non-personnel
		// cards.
		// This migration is idempotent: if the migrated data is not immediately saved,
		// the
		// same transformation will re-apply on the next sync with the same result.
		Map<String, TCard> byEquipmentId = new LinkedHashMap<>();
		// UUID → card index — the authoritative lookup used when a SarTaskResource
		// carries
		...
		for (TCard card : cards) {
			...
			// Migrate legacy PERSONNEL cards that have numberOfPersons == 0 (pre-default
			// era).
		...
		if (chart != null) {
			...
			if (ics != null) {
				for (int i = 0; i < ics.size(); i++) {
					...
					if (!name.isBlank()) {
						...
						if (chart.getIncidentCommanderEntries() != null
								&& i < chart.getIncidentCommanderEntries().size()) {
						...
						if (!icRadio.isBlank())
							card.setRadioChannel(icRadio);
						if (!icPhone.isBlank())
							card.setPhoneNumber(icPhone);
			...
			addOrgCard(wanted, byRef, byName, "org:safetyOfficer", chart.getSafetyOfficerName(),
					chart.getSafetyOfficerRadio(), chart.getSafetyOfficerPhone(), "Safety Officer");
			addOrgCard(wanted, byRef, byName, "org:pio", chart.getPublicInformationOfficerName(),
					chart.getPublicInformationOfficerRadio(), chart.getPublicInformationOfficerPhone(),
					"Public Information Officer");
			addOrgCard(wanted, byRef, byName, "org:liaisonOfficer", chart.getLiaisonOfficerName(),
					chart.getLiaisonOfficerRadio(), chart.getLiaisonOfficerPhone(), "Liaison Officer");
			addOrgCard(wanted, byRef, byName, "org:operationsChief", chart.getOperationsSectionChiefName(),
					chart.getOperationsSectionChiefRadio(), chart.getOperationsSectionChiefPhone(),
					"Operations Section Chief");
			addOrgCard(wanted, byRef, byName, "org:planningChief", chart.getPlanningSectionChiefName(),
					chart.getPlanningSectionChiefRadio(), chart.getPlanningSectionChiefPhone(),
					"Planning Section Chief");
			addOrgCard(wanted, byRef, byName, "org:logisticsChief", chart.getLogisticsSectionChiefName(),
					chart.getLogisticsSectionChiefRadio(), chart.getLogisticsSectionChiefPhone(),
					"Logistics Section Chief");
			addOrgCard(wanted, byRef, byName, "org:financeAdminChief", chart.getFinanceAdminSectionChiefName(),
					chart.getFinanceAdminSectionChiefRadio(), chart.getFinanceAdminSectionChiefPhone(),
					"Finance/Admin Section Chief");
			addOrgCard(wanted, byRef, byName, "org:documentationUnitLeader", chart.getDocumentationUnitLeaderName(),
					chart.getDocumentationUnitLeaderRadio(), chart.getDocumentationUnitLeaderPhone(),
					"Documentation Unit Leader");
			addOrgCard(wanted, byRef, byName, "org:commUnitLeader", chart.getCommunicationsUnitLeaderName(),
					chart.getCommunicationsUnitLeaderRadio(), chart.getCommunicationsUnitLeaderPhone(),
					"Communications Unit Leader");
			addOrgCard(wanted, byRef, byName, "org:commTechnician", chart.getCommunicationsTechnicianName(),
					chart.getCommunicationsTechnicianRadio(), chart.getCommunicationsTechnicianPhone(),
					"Communications Technician");
		}

		// --- Context preparer ---
		// Always maintain a T-card for the named preparer/current user, regardless of
		// whether
		// their title maps to a specific org chart slot. This is keyed by a fixed
		// sourceRef so
		...
		if (ctx != null && !safe(ctx.getCurrentUser()).isBlank()) {
			...
			preparerCard
					.setSourceRef(preparerCard.getSourceRef().isBlank() ? preparerRef : preparerCard.getSourceRef());
		...
		// --- SAR task resources ---
		// Track desired T-card status from task lifecycle (identity-keyed for dedup
		// safety).
			...
			if (resources != null) {
				...
				for (int i = 0; i < resources.size(); i++) {
					...
					if (!linkedId.isBlank() && byCardId.containsKey(linkedId)) {
						...
						setTaskSourceRef(card, ref);
						if (card.getCardType() != TCardType.PERSONNEL && isCanineTask && !leaderName.isBlank()
								&& !resNameIsLeader && card.getHandlerName().isBlank()) {
					...
					if (equipCard != null) {
						...
						if (existingPersonCard != null && existingPersonCard.getCardType() == TCardType.PERSONNEL) {
							equipCard = null;
						}
					}
					if (equipCard != null) {
		...
		// Rebuild the card list: keep manually created cards first, then source-linked
		// cards
	...
	private static final String[] DEFAULT_HEADER_LABELS = {"ICP", "Available", "Assigned", "Out of Service", "Enroute"};
	 ...
	 *
	 * <p>
	 * If no HEADER cards are present the default labels (ICP, Available, Assigned,
	 * Out of Service, Enroute) are prepended; in
	 * {@link org.sarmanagement.icsforms.model.IapPhase#PRE_OP} mode an additional
	 * "Ordered" column is inserted after "ICP". Once any HEADER card is present no
	 * defaults are added so as not to override operator customisation.
	 * </p>
	 *
	 * @param cards
	 *            mutable card list to modify in-place.
	 * @param phase
	 *            current IAP phase (used to include/exclude the Ordered column).
	 */
	static void ensureDefaultHeaderCards(List<TCard> cards, org.sarmanagement.icsforms.model.IapPhase phase) {
	 ...
	 *
	 * @param lifecycle
	 *            lifecycle value from SAR task editor.
	 * @return T-card status string ("Assigned", blank, or {@code null} for
	 *         Returned).
	 */
	...
	/**
	 * Records a lifecycle-driven status for {@code card}, keeping the
	 * highest-priority value when the same card is referenced from multiple tasks.
	 *
	 * <p>
	 * A {@code null} status means "do not touch this card's status" (used for
	 * Returned tasks so that manually-set statuses such as "At Staging" are
	 * preserved). Blank statuses are recorded as the baseline so resources can move
	 * back to available when a task is reverted to Planned. Higher-priority
	 * non-blank statuses ("Assigned") still win when the same card appears on
	 * multiple tasks.
	 * </p>
	 *
	 * <p>
	 * Priority: "Assigned" &gt; blank (available).
	 * </p>
	 */
	 ...
	 * Lookup priority:
	 * <ol>
	 * <li>Card already registered under {@code ref} in {@code byRef} (same
	 * position, update).</li>
	 * <li>Card already emitted into {@code wanted} for the same name (cross-ref
	 * dedup).</li>
	 * <li>Card existing in {@code byName} from a previous sync cycle (persist old
	 * card).</li>
	 * <li>New blank card.</li>
	 * </ol>
	 */
	private TCard findOrCreatePersonCard(String ref, String personName, Map<String, TCard> byRef,
			Map<String, TCard> byName, Map<String, TCard> wanted) {
		if (byRef.containsKey(ref)) {
		...
		// Check if we already placed this person's card into wanted under a different
		// ref.
	...
	/**
	 * Finds an existing equipment T-card for {@code resourceId} or creates a new
	 * one.
	 *
	 * <p>
	 * Lookup priority:
	 ...
	 * </ol>
	 */
	private TCard findOrCreateEquipmentCard(String ref, String resourceId, Map<String, TCard> byRef,
	...
	/**
	 * Returns the resource identifiers of all EQUIPMENT T-cards whose
	 * {@code handlerName} matches {@code handlerName} (case-insensitive). Used by
	 * the SAR task editor to suggest the handler's linked canine when creating a
	 * Canine task.
	 *
	 * @param handlerName
	...
	/**
	 * Removes duplicate {@link SarTaskResource} entries from every SAR task
	 * assignment by comparing effective names (case-insensitive). When the same
	 * resource name appears more than once in a task's resources list (which can
	 * happen after T-card deduplication merges two cards that were separately
	 * referenced), all occurrences beyond the first are dropped.
	 *
	 * <p>
	 * Call this after merging duplicate T-cards so that the SAR task display does
	 * not show the same resource twice.
	 * </p>
	 */
	...
	/**
	 * Returns a sorted list of all known resource names (personnel names and
	 * equipment identifiers) from the current T-card rack. Used to populate pick
	 * lists in the SAR task editor's Resources Assigned table.
	 *
	...
	/**
	 * Returns T-cards suitable for picking as SAR task resources (excludes header
	 * cards).
	 *
	...
	/**
	 * Returns the set of T-card resource IDs that are currently on an active ("On
	 * Task") SAR task assignment. Used by the resource picker dialog to determine
	 * which cards to hide by default (cards whose resources are already committed
	 * to a live task).
	 *
	...
	public Set<String> getOnTaskResourceIds() {
		...
		for (SarTaskAssignment task : tasks) {
			...
			if (!(lifecycle.startsWith("assigned -") || "on task".equals(lifecycle)))
				continue;
			List<SarTaskResource> resources = task.getResourcesAssigned();
			if (resources == null)
				continue;
	...
	/**
	 * Validates whether a task debriefing can be marked as completed and marks it
	 * if valid.
	 *
	 * <p>
	 * A debriefing is considered complete when all of the following are satisfied:
	 * </p>
	 * <ul>
	 ...
	 *
	 * @param task
	 *            the task assignment to validate and mark.
	...
	private void addOrgCard(Map<String, TCard> wanted, Map<String, TCard> existing, Map<String, TCard> byName,
	 ...
	 *
	 * <p>
	 * A task assignment ({@code "sar:..."}) always takes priority over an org chart
	 * slot ({@code "org:..."}). Within a sync pass, the last task to claim a card
	 * wins so that a resource reassigned from one task to another moves to the new
	 * task group in the rack view. A person may be on multiple tasks across an
	 * operational period (sequentially, not simultaneously), and the rack should
	 * reflect their current assignment.
	 * </p>
	 */
	 ...
	 * resource (same UUID or same name case-insensitive) are collapsed to one
	 * entry.
	 *
	 * <p>
	 * UUID-based deduplication (when {@code resourceId} is set) takes priority over
	 * name-based deduplication. Among duplicates the entry with a non-blank
	 * {@code resourceId} is preferred; otherwise the most specific card type wins.
	 * </p>
	 */
		...
		for (SarTaskResource res : resources) {
			...
			if (!seen.containsKey(key)) {
				...
				if (!id.isBlank() && existing.getResourceId().isBlank()) {
					seen.put(key, res);
				} else if (existing.getCardType() != null && existing.getCardType() != TCardType.PERSONNEL
	...
	/**
	 * Returns the effective display name for a T-card. Returns the display name to
	 * use when matching or listing a T-card.
	 *
	 * <p>
	 * For PERSONNEL cards the person's name ({@code personName}) is used. For all
	 * other card types (equipment, canines, aircraft, crews, etc.)
	 * {@code resourceIdentifier} is the authoritative display name — the
	 * generalised identifier: dog call sign, apparatus name, tail number, etc.
	 * </p>
	 */
	 ...
	 * This prevents overwriting operator-entered notes with role labels on every
	 * sync.
	 */
	...
	private void syncLeadResource(List<SarTaskResource> existingResources, List<SarTaskResource> scaffoldResources) {
		...
		// Find the existing resource that matches the scaffold leader by UUID or name
		// so that
		// leader-derived fields are only applied to the correct person, not to a canine
		// or
		...
		for (SarTaskResource res : existingResources) {
			...
			if (leadId.isBlank() && !leadName.isBlank() && leadName.equals(safe(res.getName()).trim().toLowerCase())) {
				target = res;
				break;
			}
		}
		...
		// Only overwrite the name from the ICS 204 scaffold when the existing resource
		// does
		// not already carry a UUID link to a canonical TCard.
		if (target.getResourceId().isBlank()) {
			target.setName(lead.getName());
		}
		// Back-fill the UUID from the existing resource so the scaffold carries it
		// forward.
	...
	private void ensureDefaults(AppData data) {
		...
		if (data.getActivityEventTypes() == null || data.getActivityEventTypes().isEmpty()) {
			...
			List<String> existing = data.getActivityEventTypes().stream().map(ActivityEventType::getId).toList();
	...
	private List<String> linkedIncidentCommanders(LinkSource source,
			org.sarmanagement.icsforms.model.Ics202Form form202, OrganizationalChart organizationalChart) {
	...
	private String linkedOperationsSectionChief(LinkSource source, Ics204Form form204,
		...
		return switch (source) {
			...
			default -> !safe(form204.getOperationsSectionChiefName()).isBlank()
					? form204.getOperationsSectionChiefName()
					: organizationalChart.getOperationsSectionChiefName();
		};
	}
	...
	private LinkSource initialLinkSource(AppData data) {
		OrganizationalChart organizationalChart = data.getOrganizationalChart();
		if (organizationalChart != null && (!organizationalChart.getIncidentCommanders().isEmpty()
	 ...
	 *
	 * <p>
	 * When a clue is added via the shared Clue Log panel and an
	 * {@code assignmentId} is set on the entry, this method appends a
	 * {@link ActivityEventType#ID_CLUE_DETECTED} entry to the matching task-linked
	 * ICS 214 form so both logs stay synchronized.
	 * </p>
	 *
	 * @param clue
		...
		for (Ics214Form log : data.getActivityLogs()) {
			if (clue.getAssignmentId().equals(log.getLinkedSarTaskAssignmentId())) {
				LocalDateTime ts = clue.getDateTimeCollected() != null
						...
						: LocalDateTime.now().withSecond(0).withNano(0);
				// Avoid duplicating entries already propagated with the same timestamp and clue
				// type.
				boolean alreadyPresent = log.getActivityLog().stream()
						.anyMatch(e -> ActivityEventType.ID_CLUE_DETECTED.equals(e.getEventTypeId())
	...
	public void recordClueInIcpActivityLog(ClueLogEntry clue) {
		...
		boolean alreadyPresent = icpLog.getActivityLog().stream()
				.anyMatch(e -> ActivityEventType.ID_CLUE_DETECTED.equals(e.getEventTypeId())
						&& ts.equals(e.getTimestamp()) && activity.equals(safe(e.getNotableActivity())));
		if (alreadyPresent) {
			return;
		}
		ActivityLogEntry entry = new ActivityLogEntry();
	...
	/**
	 * Returns {@code true} when {@code value} looks like a phone number rather than
	 * a radio channel name or frequency.
	 *
	 * <p>
	 * A value is treated as a phone number when — after stripping common
	 * phone-number formatting characters ({@code - . ( ) + space}) — the remaining
	 * string contains at least 7 digit characters and no other non-digit
	 * characters. Values that contain letters, or that have fewer than 7 digits
	 * after stripping (e.g. short radio channel codes like "Ch 5" or "UHF-3"), are
	 * treated as radio channel identifiers. VHF/UHF frequencies with a decimal
	 * point (e.g. "155.340") are also rejected because they contain a decimal
	 * separator that survives stripping and they have fewer than 7 digits total.
	 * </p>
	 *
	 * @param value
	 *            string to classify; {@code null} or blank returns {@code false}.
	...
	static boolean looksLikePhoneNumber(String value) {
		...
		// Strip characters that appear in formatted phone numbers but not channel
		// names.
	...
	/**
	 * Adds the given T-card as a resource entry to the specified SAR task
	 * assignment.
	 *
	 * <p>
	 * If the task's {@code resourcesAssigned} list already contains an entry with
	 * the same name (case-insensitive) the call is a no-op to avoid creating
	 * duplicates.
	 * </p>
	 *
	 * @param card
	 *            T-card to add.
	 * @param task
	 *            target SAR task assignment.
	 * @param asLeader
	 *            when {@code true}, sets the card as the task's leader instead of
	 ...
	 *
	 * <p>
	 * The {@code roleLabel} must match one of the values returned by
	 * {@link #getOrgChartRoleLabels()}. If the label is not recognised the call is
	 * a no-op.
	 * </p>
	 *
	 * @param roleLabel
	 *            human-readable role label (e.g. {@code "Safety Officer"}).
	 * @param personName
	 *            name to assign to the role.
	 */
	public void setOrgChartRoleName(String roleLabel, String personName) {
		...
		switch (roleLabel) {
			case "Incident Commander" -> {
				List<String> ics = chart.getIncidentCommanders();
				if (!ics.contains(name)) {
					ics.add(name);
					chart.setIncidentCommanders(ics);
				}
			}
	...
	public static List<String> getOrgChartRoleLabels() {
		return List.of("Incident Commander", "Safety Officer", "Public Information Officer", "Liaison Officer",
				"Operations Section Chief", "Planning Section Chief", "Logistics Section Chief",
				"Finance/Admin Section Chief", "Documentation Unit Leader", "Communications Unit Leader",
				"Communications Technician", "Staging Area Manager", "Resources Unit Leader", "Situation Unit Leader",
				"Demobilization Unit Leader", "Supply Unit Leader");
	}
}
