 ...
 *
 * <p>
 * Every real-world resource deployed to or requested for an incident is
 * represented by exactly one {@code IncidentResource} instance. Forms and
 * operational-period assignments reference resources by their
 * {@link #getResourceId() resourceId} rather than by copying field values. This
 * ensures that edits to a resource propagate to every live reference
 * automatically, while finalized historical documents preserve their own
 * snapshots.
 * </p>
 *
 * <p>
 * The concrete subclasses correspond to the ICS 219 T-card categories:
 * </p>
 * <ul>
 ...
 * <li>{@link EquipmentResource} — 219-3 engine / 219-7 equipment / 219-8
 * misc</li>
 * <li>{@link AircraftResource} — 219-4 helicopter / 219-6 fixed-wing or
 * UAS</li>
 * <li>{@link GenericResource} — 219-10 generic</li>
 * </ul>
 *
 * <p>
 * Stable fields (those that describe the resource itself, not its
 * operational-period role) are stored here. Operational-period-specific values
 * (radio channel, check-in time, assigned status) live in
 * {@link OperationalPeriodAssignment}.
 * </p>
 */
@JsonIgnoreProperties(ignoreUnknown = true)
@JsonTypeInfo(use = JsonTypeInfo.Id.NAME, property = "resourceType")
@JsonSubTypes({@JsonSubTypes.Type(value = PersonnelResource.class, name = "PERSONNEL"),
...
public abstract class IncidentResource {
	 ...
	 *
	 * <p>
	 * For personnel this is the person's full name. For canines this is the
	 * canine's name/call-sign (not the handler's name). For equipment/aircraft this
	 * is the unit identifier or description.
	 * </p>
	 */
	...
	/**
	 * Creates a new resource entity with a freshly generated
	 * {@link #getResourceId() resourceId}.
	 */
	 ...
	 * deserialization or migration; do not reassign once the resource is referenced
	 * from other objects.
	 *
	 * @param resourceId
	 *            stable UUID string.
	 ...
	 *
	 * @param displayName
	 *            display name; {@code null} treated as blank.
	 */
	 ...
	 *
	 * @param homeAgency
	 *            home agency; {@code null} treated as blank.
	 */
	 ...
	 *
	 * @param phoneNumber
	 *            phone number; {@code null} treated as blank.
	 */
	 ...
	 *            resource identifier; {@code null} treated as blank.
	 */
	 ...
	 *
	 * @param notes
	 *            notes; {@code null} treated as blank.
	 */
	...
	/**
	 * Sets the last-updated instant (used during deserialization or explicit
	 * updates).
	 *
	 * @param updatedAt
	...
	/**
	 * Returns the {@link TCardType} that corresponds to this resource subtype, for
	 * use when bridging to legacy T-card rendering code.
	 *
