...
/**
 * Builds the canonical personnel-directory rows shared by the T-card Directory
 * View and the ICS 205A Communications List export.
 */
public final class ResourceDirectorySource {
	...
	private static final java.util.Set<String> NAME_SUFFIXES = java.util.Set.of("jr", "jr.", "sr", "sr.", "ii", "iii",
			"iv", "v");
	...
	/**
	 * Builds directory rows from the current incident document and supplied T-card
	 * list.
	 *
	 * <p>
	 * This overload lets the T-card UI render the same canonical directory rows
	 * from the in-memory table model before those edits are pushed back into
	 * {@link AppData}.
	 * </p>
	 *
	 * @param data
	 *            incident document
	 * @param cards
	 *            T-cards to project into directory rows
	 * @return canonical directory rows
	 */
	public static List<ResourceDirectoryEntry> build(AppData data, List<TCard> cards) {
		...
		for (TCard card : cards) {
			...
			rows.add(new ResourceDirectoryEntry(card, name, assignedPosition(card, safeData, tasksById),
					safe(card.getHomeState()), safe(card.getHomeAgency()), assignment(card, tasksById),
					safe(card.getStatus()), contactMethods(card)));
		}
		return rows;
	}
	...
	private static String assignedPosition(TCard card, AppData data, Map<String, SarTaskAssignment> tasksById) {
		...
		if ("context:preparer".equals(ref)) {
			...
			return context == null ? "Preparer" : coalesce(context.getCurrentUserPositionTitle(), "Preparer");
		}
