 ...
 *
 * <p>
 * The minimal implementation focuses on the 219-5 Personnel card. All other
 * card types are scaffolded with a common set of fields and a {@link TCardType}
 * discriminator. Special mappings: canine resources are stored as
 * {@link TCardType#EQUIPMENT} (219-7, yellow — canines are treated as working
 * assets/equipment in ICS); drone/fixed-wing resources are stored as
 * {@link TCardType#FIXED_WING} (219-6).
 * </p>
 */
...
public class TCard {
	...
	/**
	 * Stable UUID assigned once at creation and never changed. Used by
	 * {@link SarTaskResource#getResourceId()} to reference this card without
	 * relying on mutable display-name strings. Initialized to {@code null}; a UUID
	 * is generated on first access via {@link #getResourceId()} to ensure legacy
	 * records deserialized without a {@code resourceId} field receive a stable
	 * value rather than a new random one each time.
	 */
	 ...
	 * Format conventions:
	 * <ul>
	 * <li>{@code "org:<fieldKey>"} — org chart staff position, e.g.
	 * {@code "org:safetyOfficer"}</li>
	 * <li>{@code "sar:<assignmentId>:<resourceIndex>"} — SAR task resource, index 0
	 * being the task leader</li>
	 * </ul>
	 * Blank when the card was created manually or imported from CSV.
	 * </p>
	 */
	 ...
	 * handler's personnel card. A handler may be linked to more than one canine
	 * card. Blank when not applicable.
	 */
	private String handlerName = "";

	/**
	 * Number of persons represented by this T-card. Meaningful for CREW cards (a
	 * crew of N people) and PERSONNEL cards (always 1). Zero means unspecified.
	 */
	...
	/**
	 * Returns the stable resource identifier (UUID) for this card. Used by
	 * {@link SarTaskResource} to reference this card without relying on names.
	 *
	...
	/**
	 * Sets the stable resource identifier. Should only be called during
	 * deserialization.
	 *
	 * @param resourceId
	 *            resource UUID string; blank/null generates a new UUID.
	 */
	public void setResourceId(String resourceId) {
		this.resourceId = (resourceId == null || resourceId.isBlank()) ? UUID.randomUUID().toString() : resourceId;
	}
	 ...
	 *
	 * @param homeState
	 *            home state (two-letter abbreviation).
	 */
	...
	/**
	 * Returns the source reference identifying the incident object (org chart
	 * position or SAR task resource) that generated this T-card, or blank for
	 ...
	 *
	 * @param sourceRef
	 *            source reference; {@code null} is treated as blank.
	 */
	 ...
	 *
	 * <p>
	 * Used to group canine (and other resource) cards below their handler's
	 * personnel card in the rack view. Blank when not applicable.
	 * </p>
	 *
	 ...
	 *
	 * @param handlerName
	 *            handler name; {@code null} is treated as blank.
	 */
	 ...
	 * For PERSONNEL cards the effective count is always 1; use
	 * {@link #personCount()} when computing headcounts.
	 *
	...
	/**
	 * Sets the number of persons for this card. Values below zero are clamped to
	 * zero.
	 *
	 * @param numberOfPersons
	 *            number of persons (&ge; 0).
	 */
	...
	/**
	 * Returns the number of people contributed by this card to an assignment
	 * headcount.
	 *
	 * <ul>
	 ...
	 * other card types (equipment, canines, aircraft, etc.),
