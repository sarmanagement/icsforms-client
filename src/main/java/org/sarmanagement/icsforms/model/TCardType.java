 ...
 *
 * <p>
 * Each value corresponds to a standard ICS 219 card colour and resource
 * category. Some types carry an optional SAR-mode label that is displayed
 * instead of the standard label when the incident is running in SAR mode (see
 * {@link #getLabel(boolean)}).
 * </p>
 */
public enum TCardType {
	...
	/**
	 * 219-7 Equipment card (yellow). Used for ground equipment. In SAR mode the
	 * label includes the canine note because canines are treated as working
	 * assets/equipment in ICS SAR operations. JSON alias {@code "DOZER"} accepted
	 ...
	 *
	 * <p>
	 * Application-defined extension card for general-purpose resources that do not
	 * fit any of the standard ICS 219-1 through 219-8 categories.
	 * </p>
	 */
	...
	/**
	 * Optional SAR-mode label; {@code null} when the standard label applies in all
	 * modes.
	 */
	 ...
	 *
	 * <p>
	 * When {@code sarMode} is {@code true} and this type has a SAR-specific label,
	 * the SAR label is returned; otherwise the standard label is returned.
	 * </p>
	 *
	 * @param sarMode
	 *            {@code true} when the incident is running in SAR mode.
	...
	/**
	 * Returns {@code true} when {@code candidate} matches either the standard label
	 * or the SAR-mode label of this type (case-insensitive).
	 *
	 * <p>
	 * Useful when deserialising a stored label whose mode-context is unknown.
	 * </p>
	 *
	 * @param candidate
	 *            the label string to test.
	 * @return {@code true} when the candidate matches any label variant for this
	 *         type.
	 */
	public boolean matchesLabel(String candidate) {
		...
		return label.equalsIgnoreCase(candidate) || (sarLabel != null && sarLabel.equalsIgnoreCase(candidate));
	}
