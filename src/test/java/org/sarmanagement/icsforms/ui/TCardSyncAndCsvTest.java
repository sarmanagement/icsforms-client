...
/**
 * Tests for T-card cross-linking (org chart and SAR tasks) and CSV import
 * helpers.
 */
class TCardSyncAndCsvTest {
	...
	void syncTCardsRemovesCardWhenOrgChartPositionCleared() {
		...
		assertNull(findByRef(data.getTCards(), "org:safetyOfficer"), "Card should be removed when position is cleared");
	}
	...
	void syncTCardsCreatesCardsForAllNamedOrgChartPositions() {
		...
		String[] refs = {"org:safetyOfficer", "org:pio", "org:liaisonOfficer", "org:operationsChief",
				"org:planningChief", "org:logisticsChief", "org:financeAdminChief", "org:documentationUnitLeader"};
		for (String ref : refs) {
	...
	/**
	 * Regression: a stale EQUIPMENT card whose {@code resourceIdentifier} matches
	 * the handler's name (created by an older sync version) must not produce a
	 * duplicate. After sync the handler should appear exactly once as PERSONNEL,
	 * and the stale EQUIPMENT card (which is no longer referenced) must be removed.
	 */
	@Test
	void syncTCardsDoesNotDuplicateHandlerWhenStaleEquipmentCardExists() {
		...
		long personnelCount = cards.stream()
				.filter(c -> c.getCardType() == TCardType.PERSONNEL && "John Smith".equals(c.getPersonName())).count();
		assertEquals(1, personnelCount, "Handler must appear exactly once as PERSONNEL; found " + personnelCount);

		long equipCount = cards.stream()
				.filter(c -> (c.getCardType() == TCardType.EQUIPMENT || c.getCardType() == TCardType.MISC_EQUIPMENT)
						&& "John Smith".equals(c.getResourceIdentifier()))
				.count();
		assertEquals(0, equipCount, "Stale EQUIPMENT card with handler name must be removed after sync");
	}

	/**
	 * Regression: a canine T-card that stores the dog's name in
	 * {@code resourceIdentifier} (the generalised identifier for all non-personnel
	 * resources) must remain as EQUIPMENT after {@code syncTCards()} and must not
	 * flip to PERSONNEL or display the handler's name. Legacy data that had the
	 * name in {@code personName} with a blank {@code resourceIdentifier} is
	 * automatically migrated by {@code syncTCards()} to use
	...
	void syncTCardsPreservesCanineCardTypeWhenNameStoredInPersonName() {
		...
		// Canine T-card: legacy format — name stored in personName, resourceIdentifier
		// blank.
		...
		// The canine appears in resourcesAssigned with the dog's name and EQUIPMENT
		// type.
		...
		Optional<TCard> rex = cards.stream().filter(c -> "Rex".equalsIgnoreCase(c.getResourceIdentifier())).findFirst();
	...
	/**
	 * Regression: when a canine is entered manually on the SAR task resource list
	 * with cardType=EQUIPMENT but no existing TCard exists for it yet (cold-start /
	 * post-restart), syncTCards must create an EQUIPMENT card, not a PERSONNEL
	 * card. Previously it fell through to findOrCreatePersonCard, which created a
	 * PERSONNEL card bearing the canine's name and then set res.cardType to
	 * PERSONNEL — causing handler name bleed-through on the next sync.
	 */
	@Test
	void syncTCardsCreatesEquipmentCardForManuallyTypedEquipmentResource() {
		...
		Optional<TCard> buddy = cards.stream().filter(c -> "Buddy".equalsIgnoreCase(c.getResourceIdentifier()))
				.findFirst();
		...
		long handlerCount = cards.stream()
				.filter(c -> c.getCardType() == TCardType.PERSONNEL && "Jane Doe".equalsIgnoreCase(c.getPersonName()))
				.count();
	 ...
	 * task group (sourceRef starts with "sar:"), not under ICP (sourceRef
	 * "org:...").
	 */
	@Test
	void syncTCardsTaskRefOverridesOrgChartRef() {
		...
		long aliceCount = cards.stream().filter(c -> "Alice Johnson".equalsIgnoreCase(c.getPersonName())).count();
		assertEquals(1, aliceCount, "Alice must have exactly one T-card");

		TCard alice = cards.stream().filter(c -> "Alice Johnson".equalsIgnoreCase(c.getPersonName())).findFirst().get();
	...
	/**
	 * A resource entry with a stale non-PERSONNEL cardType whose name matches a
	 * known person must be corrected to PERSONNEL rather than creating a phantom
	 * equipment card.
	 */
	@Test
	void syncTCardsCorrectedStaleEquipmentCardTypeForKnownPerson() {
		...
		// resourcesAssigned has two stale entries: one EQUIPMENT and one PERSONNEL for
		// same name.
		SarTaskAssignment task = new SarTaskAssignment();
		...
		long handlerCardCount = cards.stream().filter(c -> "John Doe".equalsIgnoreCase(c.getPersonName())).count();
		long phantomEquipCount = cards.stream().filter(
				c -> c.getCardType() != TCardType.PERSONNEL && "John Doe".equalsIgnoreCase(c.getResourceIdentifier()))
				.count();
	...
	void syncTCardsPersonnelCardHasDefaultNumberOfPersonsOne() {
		...
		TCard card = data.getTCards().stream().filter(c -> "Bob Smith".equalsIgnoreCase(c.getPersonName())).findFirst()
				.orElse(null);
		assertNotNull(card);
		assertEquals(1, card.getNumberOfPersons(), "PERSONNEL card must default to numberOfPersons = 1");
	}

	/**
	 * Existing PERSONNEL cards with numberOfPersons == 0 (legacy data) must be
	 * migrated to 1.
	 */
	@Test
	...
	void inferCardTypeRecognisesKeywords() {
		// Canine and handler → 219-7 Equipment (canines are working assets/equipment in
		// ICS)
	...
	/**
	 * On a canine task the resource list may contain the handler's own name typed
	 * as EQUIPMENT (a legacy UI artefact). That entry must be treated as PERSONNEL
	 * — not used to create a canine T-card — and the canine card must not have the
	 * handler's name as its identifier.
	 */
	@Test
	void syncTCardsDoesNotConflateHandlerNameWithCanineResource() {
		...
		// Resource list has the handler's own name typed as EQUIPMENT (old bug
		// artefact)
		...
		long phantomEquip = cards.stream().filter(
				c -> c.getCardType() != TCardType.PERSONNEL && "Jane Doe".equalsIgnoreCase(c.getResourceIdentifier()))
				.count();
		assertEquals(0, phantomEquip, "No equipment card may have the handler's name as resourceIdentifier");

		// Canine card must exist with correct name and handler link.
		TCard canineCard = cards.stream().filter(c -> "K9 Rex".equalsIgnoreCase(c.getResourceIdentifier())).findFirst()
				.orElse(null);
		assertNotNull(canineCard, "Canine T-card must exist");
		assertEquals(TCardType.EQUIPMENT, canineCard.getCardType());
		assertEquals("Jane Doe", canineCard.getHandlerName(), "Canine card must reference the handler's name");
	}

	/**
	 * A resource assigned to a second task must appear under that task in the rack
	 * view (last-assignment-wins), not remain locked to the first task's ref.
	 */
	@Test
	void syncTCardsUpdatesSourceRefWhenResourceReassignedToAnotherTask() {
		...
		List<TCard> cards = data.getTCards();
		TCard bobCard = cards.stream().filter(c -> "Bob Jones".equalsIgnoreCase(c.getPersonName())).findFirst()
				.orElse(null);
		...
		assertTrue(bobCard.getSourceRef().startsWith("sar:task-bravo"),
				"Bob's sourceRef must reflect the last task that claimed him, got: " + bobCard.getSourceRef());
	}
	...
	private static final class TestAppController {
		static AppController create(AppData data) {
			org.sarmanagement.icsforms.persistence.LocalRepository repo = new org.sarmanagement.icsforms.persistence.LocalRepository(
					java.nio.file.Path.of(System.getProperty("java.io.tmpdir"), "tcard_test.json"));
			org.sarmanagement.icsforms.pdf.PdfExportService exportService = new org.sarmanagement.icsforms.pdf.PdfExportService();
			org.sarmanagement.icsforms.validation.IncidentValidator validator = new org.sarmanagement.icsforms.validation.IncidentValidator();
