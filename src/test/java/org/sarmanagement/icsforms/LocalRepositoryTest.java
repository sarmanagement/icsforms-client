...
/**
 * Focused tests for persistence, validation, SAR linkage, and PDF smoke
 * exports.
 */
class LocalRepositoryTest {

	/**
	 * Verifies that workspace JSON preserves schema version, shared context, and
	 * linked assignment data.
	 *
	 * @throws Exception
	 *             when the temp file cannot be used.
	 */
	@Test
	void roundTripPersistsAndLoads() throws Exception {
		...
		assertEquals(SarTaskSupport.RESOURCE_TYPE_CANINE,
				loaded.getForm204().getResourcesAssigned().get(0).getResourceType());
	...
	/**
	 * Verifies corrupt primary files recover gracefully from the last known-good
	 * backup.
	 *
	 * @throws Exception
	 *             when temp file setup fails.
	 */
	@Test
	...
	void validatorReportsConditionalAndOrderingFailures() {
		...
		assertTrue(
				messages.stream().anyMatch(message -> message.field().equals("ics204.divisionGroupSupervisorContact")));
	}

	/**
	 * Verifies branch selection requires branch director data instead of
	 * division/group supervisor data.
	 */
	@Test
	void validatorUsesBranchSpecificSupervisorFields() {
		...
		assertFalse(
				messages.stream().anyMatch(message -> message.field().equals("ics204.divisionGroupSupervisorName")));
	}

	/**
	 * Verifies ICS 204 resource linkage creates a matching SAR scaffold entry with
	...
	void resourceAssignmentMapsToSarScaffold() {
		...
		SarTaskAssignment task = SarTaskAssignment.fromResourceAssignment(resource, data.getIncidentContext(),
				data.getForm204());
	...
	/**
	 * Verifies syncing SAR tasks preserves debrief details while refreshing shared
	 * assignment data.
	 *
	 * @throws Exception
	 *             when temp file setup fails.
	...
	void syncSarTasksPreservesDebriefData() throws Exception {
		Path tempDir = Files.createTempDirectory("icsforms");
		AppController controller = new AppController(sampleData(),
				new LocalRepository(tempDir.resolve("incident.json")), new PdfExportService(new Ics201PdfRenderer(),
	...
	/**
	 * Verifies the org chart tab data links bidirectionally with ICS 202/204 and
	 * shared preparer roles.
	 *
	 * @throws Exception
	 *             when temp file setup fails.
	 */
	...
	void organizationalChartLinksAcrossTabs() throws Exception {
		...
		assertEquals(List.of("IC Alpha", "IC Bravo"),
				controller.getData().getOrganizationalChart().getIncidentCommanders());
	...
	void pdfExportsAreGenerated() throws Exception {
		...
		PdfExportService exportService = new PdfExportService(new Ics201PdfRenderer(), new Ics202PdfRenderer(),
				new Ics204PdfRenderer(), new SarTaskAssignmentPdfRenderer());
	...
	/**
	 * Verifies that the IAP bundle export produces a single merged PDF file with
	 * the incident name and operational period encoded in the filename, and that
	 * the individual component PDFs are removed after merge.
	 */
	@Test
	void iapBundleExportProducesMergedPdf() throws Exception {
		...
		assertTrue(
				bundlePath.getFileName().toString().contains("Test_Incident")
		...
		// The merged document should have at least as many pages as the individual
		// forms.
